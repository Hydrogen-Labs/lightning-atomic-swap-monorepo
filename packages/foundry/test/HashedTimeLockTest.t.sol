...
contract HashedTimelockTest is Test {
    ...
    function testFuzzWithdraw(uint256 _amount, bytes32 _preimage) public {
    ...
    function testFuzzWithdrawWithBounty(uint256 _amount, bytes32 _preimage, uint64 _fee) public {
        vm.assume(_amount > 0.1 ether);
        vm.assume(_fee > 0);
        vm.assume(_fee < 1000); // Reasonable fee range

        vm.deal(sender, _amount); // Provide the sender with the fuzzed amount
        ...
        vm.fee(_fee);
        uint256 bounty = htlc.calculateBounty();
        bool success = htlc.withdrawWithBounty(contractId, _preimage);
        assertTrue(success);

        // Verify receiver's balance has increased by the contract amount minus bounty
        assertEq(receiver.balance, _amount - bounty);
    }
    ...
    function testDuplicateHashlock() public {
        // Create first contract with a hashlock
        htlc.newContract{value: amount}(receiver, hashlock, timelock);
        vm.stopPrank();

        // Try to create second contract with same hashlock
        vm.startPrank(sender);
        vm.expectRevert();
